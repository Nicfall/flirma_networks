...
ps.sid.ord <- ordinate(ps.sid,"PCoA",distance="bray")
plot_ordination(ps.sid, ps.sid.ord, color ="zone")+
...
seq.sid <- data.frame(ps.sid@otu_table)
samdf.sid <- data.frame(ps.sid@sam_data)

dist.sid <- vegdist(seq.sid)
##zone
bet.sid <- betadisper(dist.sid,samdf.sid$zone)
#anova(bet.ps)
permutest(bet.sid,pairwise=TRUE,permutations=999)
plot(bet.ps) #0.07, trend

adonis2(seq.sid ~ zone, data=samdf.sid, permutations=999) #p<0.01**

##time
bet.sid <- betadisper(dist.sid,samdf.sid$time)
#anova(bet.ps)
permutest(bet.sid,pairwise=TRUE,permutations=999)
plot(bet.ps) #ns

adonis2( ~ time, data=samdf.sid, permutations=999) #p<0.001***

pairwise.adonis(seq.sid, factors=samdf.sid$time, permutations=999)
```

Relative abundance

```{r stats by site rel}
seq.ps.rel <- data.frame(ps.sid@otu_table)
