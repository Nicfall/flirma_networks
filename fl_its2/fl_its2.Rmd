...
# tax_table()   Taxonomy Table:    [ 75 taxa by 4 taxonomic ranks ]
...
##for colors planning:
##need 3 for A - greys
# pal <- hcl.colors(3, "Grays")
# show_col(pal)
# pal #took last three:
# "#1B1B1B" "#969696" "#F9F9F9"

##need 5 for B - blue
##need 5 for C - green
# 
# pal <- hcl.colors(10,"Cork")
# show_col(pal)
# pal
#"#00294D","#195481","#5882AF","#9AB0CE","#D6DDE9","#D3E0D2","#95B792","#548C4E","#1F5D12","#002F00"

#need 1 for D - brown
#"#B8773A"

#majority.seq.colors <- c("#492900","#B8773A","#F6D2BE",
                         #   "#E2FCFF","#B5D9FC","#8CB6DD","#7293B2","#587189","#405162","#2A333C","#181818","#333325","#515138","#71704C","#939261","#B6B577","#D9D8A2","#FCFCD3",
...
gg.bar.pre <- plot_bar(ps.maj.sid.pre.rel.sz, fill="Majority_ITS2_collapsed")+
    ggtitle("Pre")+
    theme_cowplot()+
    theme(axis.text.x=element_text(angle=90,hjust=1))+
    xlab("")+
...
```{r}
#by sample
ps.sid.pre.rel <- transform_sample_counts(ps.sid.pre, function(x) x / sum(x))

plot_bar(ps.sid.pre.rel, fill="Majority_ITS2_sequence")+
    ggtitle("Pre")+
    theme_cowplot()+
    theme(axis.text.x=element_text(angle=90,hjust=1))+
    xlab("")
...
ps.sid.pre.rel <- transform_sample_counts(ps.sid.pre, function(x) x / sum(x))
ps.sid.pre.rel.sz <- merge_samples(ps.sid.pre.rel, "site_zone")
ps.sid.pre.rel.sz.rel <- transform_sample_counts(ps.sid.pre.rel.sz, function(x) x / sum(x))

bar.sid.pre.sz <- plot_bar(ps.sid.pre.rel.sz.rel, fill="ITS2_type_profile")+
    ggtitle("Pre")+
    theme_cowplot()+
...
Mo'orea stuff to update/delete
