...
setwd("/Volumes/GoogleDrive/My Drive/Flirma/flirma_networks/fl_16s/01.generate_asv_table")
ps.cleanest <- readRDS("ps.cleanest.rds")
taxa <- readRDS("fl16s.rd3.taxa.newids.rds")
samdf <- read.csv("fl_sample_info - 16s_all_newvars.csv")
row.names(samdf) <- samdf$sample_num
```
...
fewer <- subset(total, total <6500)
#21 samples to remove, that's ~95% of samples which seems good
fewer

row.names.remove <- c("1196","1200","1206","1207","1222","1236","1238","1143","1145","1146","1149","703","1171","1176","1071","1075","1086","1088","827","850","1119")
...
#saveRDS(ps.rare,file="ps.cleanest.rare6500.rds")
...
*Note:* Using these two phyloseq objects (ps.less, ps.rare) in alpha diversity, because authors say not to use trimmed objects in alpha diversity (trimming below)
...
```{r rare}
ps.rare <- readRDS("ps.cleanest.rare6500.rds")
...
ps.otu <- data.frame(ps.less@otu_table)
...
seq.trim.allinfo <- purgeOutliers(seq.formcmc,count.columns=3:11982,sampleZcut=-2.5,otu.cut=0,zero.cut=0.00597)
# [1] "samples with counts below z-score -2.5 :"
# character(0)
# [1] "zscores:"
# named numeric(0)
# [1] "OTUs with counts in 0.00597 of samples:"
# 
# FALSE  TRUE 
#  1267 10713 

#remove sample info
seq.trim <- seq.trim.allinfo[,3:10715]
...
# otu_table()   OTU Table:         [ 10713 taxa and 335 samples ]
# sample_data() Sample Data:       [ 335 samples by 11 sample variables ]
# tax_table()   Taxonomy Table:    [ 10713 taxa by 8 taxonomic ranks ]

#saveRDS(ps.trim,file="ps.cleanest.trim.rds")
```

Trimmed & rarefied for checks

Ran once then just reading in file next time

```{r, eval=F}
seqtab.rare2 <- rrarefy(seq.trim,sample=6500)
#rarecurve(seqtab.rare2,step=500,label=FALSE)

#phyloseq object but rarefied
ps.trim.rare <- phyloseq(otu_table(seqtab.rare2, taxa_are_rows=FALSE), 
                    sample_data(samdf), 
                    tax_table(taxa))
ps.trim.rare
...
ps.trim.rare #10711 taxa

#saveRDS(ps.trim.rare,file="ps.cleanest.trim.rare.rds")
```

Plot trimmed

```{r plot trimmed}
ps.trim <- readRDS(file="ps.cleanest.trim.rds")
...
```{r CLR}
ps.less.clr <- microbiome::transform(ps.less,"clr")
ps.less.clr.ord <- ordinate(ps.less.clr,"PCoA","euclidean")

plot_ordination(ps.less.clr,ps.less.clr.ord,color="year")+
  stat_ellipse()

plot_ordination(ps.less.clr,ps.less.clr.ord,color="year",shape="zone")+
  facet_wrap(spp~site)+
  stat_ellipse()

#saveRDS(ps.less.clr,file="ps.cleanest.less.clr.rds")
```
