...
ps.clean <- readRDS("../01.generate_asv_table/ps.cleanest.trim.rds")
...
gg.clean.sid <- plot_ordination(ps.clean.sid, ord.clean.sid,axes=c(1,2),color="time",shape="zone",)+
  stat_ellipse()+
  scale_color_manual(values=c("#3B0F70","#FA7F53","#A8327D"))+
  theme_cowplot()

df.gg.clean.sid <- data.frame(gg.clean.sid["data"])

<<<<<<< HEAD
ggplot(df.gg.clean.sid,aes(x=data.Axis.1,y=data.Axis.2,shape=data.site))+
  stat_ellipse(aes(linetype=data.site,color=data.time))+
  scale_color_manual(values=c("#3B0F70","#FA7F53","#A8327D"))+
  #facet_wrap(~data.time)+  
  geom_point(aes(color=data.zone,fill=data.time))+
  ylab("Shannon index")+
  xlab("Time point")+
  theme_cowplot()+
  #scale_color_manual(values=c("white","grey"))+
  #scale_shape_manual(name="Site",values=c(21,22,23))+
  scale_color_manual(values=c("blue","green"))+
  #scale_color_manual(values=c("black","grey","white"))+
  #scale_shape_manual(values=c(22,24,25))+
=======
ggplot(df.gg.clean.sid,aes(x=data.Axis.1,y=data.Axis.2,color=data.time,shape=data.site))+
  stat_ellipse()+
  #facet_wrap(~data.time)+  
  geom_point()+
  ylab("Shannon index")+
  ...
  #scale_color_manual(values=c("blue","green"))+
  #scale_color_manual(values=c("black","grey","white"))+
  scale_shape_manual(values=c(8,4,9))+
>>>>>>> bf0c95b70c7746c04ef9d691117cf00e17bf6411
...
```{r}
seq.clean.sid.pre.lk1 <- data.frame(otu_table(ps.clean.sid.pre.lk1))
samdf.clean.sid.pre.lk1 <- data.frame(sample_data(ps.clean.sid.pre.lk1))
row.names(seq.clean.sid.pre.lk1)==row.names(samdf.clean.sid.pre.lk1)

dist.clean.sid.pre.lk1 <- vegdist(seq.clean.sid.pre.lk1)
beta.clean.sid.pre.lk1 <- betadisper(dist.clean.sid.pre.lk1,samdf.clean.sid.pre.lk1$zone,bias.adjust = TRUE,type="median")
anova(beta.clean.sid.pre.lk1) #not sig
#permutest(beta.clean.sid.pre.lk1, pairwise = FALSE, permutations = 999) #says the same thing
plot(beta.clean.sid.pre.lk1)

adonis(seq.clean.sid.pre.lk1 ~ zone, data=samdf.clean.sid.pre.lk1, permutations=999) #not sig
```
